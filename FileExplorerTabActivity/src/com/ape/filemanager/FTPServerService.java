...
public class FTPServerService extends Service implements Runnable {
    ...
    private static final int MSG_SERVER_STARTED = 1;
        ...
        public void dispatchMessage(android.os.Message msg)
        {
            switch (msg.what)
            {
                case MSG_SERVER_STARTED:
                    myLog.l(Log.INFO, "dispatchMessage, MSG_SERVER_STARTED");
                    CharSequence tickerText = getString(R.string.notif_server_starting);
                    Toast.makeText(getApplicationContext(), tickerText, Toast.LENGTH_SHORT).show();
                    break;
    ...
    private void setupNotification() {
        ...
        int icon = R.drawable.notification1;
        //CharSequence tickerText = getString(R.string.notif_server_starting);
...
//todo        
      ...
      PendingIntent intent = PendingIntent.getActivity(this, 0, new Intent(this, FileExplorerTabActivity.class), 0);
      //PendingIntent点击通知后所跳转的页面
      builder.setContentTitle("Bmob Test");
      builder.setContentText("message");
      builder.setSmallIcon(icon);
      builder.setContentIntent(intent);
      Notification notification = builder.getNotification();//builder -> notification
      notification.flags |= Notification.FLAG_ONGOING_EVENT;//点击通知后通知消失
      manager.notify(1,notification);//run  notification
        

        myLog.l(Log.INFO, "Notication setup done");
    }

    private void clearNotification() {
        stopForeground(true);
        myLog.l(Log.INFO, "Cleared notification");
    }
