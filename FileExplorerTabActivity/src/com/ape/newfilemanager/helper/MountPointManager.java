...
public final class MountPointManager
{
	...
	public void init(Context context)
	{
		...
		if (volumePathList != null)
		{
			...
			for (int i = 0; i < volumePathList.length; i++)
			{
			    ...
			    int stringId = mountPoint.mIsExternal ? R.string.storage_sd_card_overlay : R.string.phone_storage;
                    mountPoint.mDescription = mContext.getString(stringId);
				}
	...
	public List<FileInfo> getMountPointFileInfo()
	{
		...
		for (MountPoint mp : mMountPathList)
		{
			if (mp.mIsMounted)
			{
			    FileInfo info = Util.GetFileInfo(mp.mPath);
			    if (info == null)
			        continue;
			    info.displayName = mp.mDescription;
			    info.isExternal = mp.mIsExternal;

			    SDCardInfo storageInfo = null;// Util.getSDCardInfoByPath(mp.mPath);
			    if(mp.mPath != null && mp.mPath.contains("/storage/emulated")){
				List<String> paths = new ArrayList<String>(1);
				paths.add(mp.mPath);
				storageInfo = Util.getSDCardInfoFromPaths(mContext, paths);
			    } else{
				storageInfo = Util.getSDCardInfoByPath(mp.mPath);
			    }
			    if (storageInfo != null)
			    {
