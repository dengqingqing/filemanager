...
import com.ape.newfilemanager.helper.Util.SDCardInfo;
...
public class FileOperationHelper {
	...
	// todo
	private static ArrayList<FileInfo> mCurFileNameList = new ArrayList<FileInfo>();
	...
	public FileOperationHelper(IOperationProgressListener l) {
		...
		mMediaProviderHelper = new MediaStoreHelper(
				mOperationListener.getContext());
		mPasteMediaStoreHelper = new PasteMediaStoreHelper(mMediaProviderHelper);
		mDeleteMediaStoreHelper = new DeleteMediaStoreHelper(
	...
	public boolean EndMove(String path) {
		...
		if (mCurFileNameList.size() > 0) {
			...
			String srcDriver = Util.getRootPathFromFilePath(mCurFileNameList
					.get(0).filePath);
	...
	private void asnycExecute(Runnable r) {
		...
		new AsyncTask() {
			...
			protected void onPostExecute(Object result) {
				...
				Log.i(LOG_TAG, "asnycExecute, OperationListener:"
						+ mOperationListener);
				if (mOperationListener != null) {
					mOperationListener.onFinish();
				}
		...
		try {
			...
			if (ret) {
				if (isDir) {
					...
					mMediaProviderHelper.updateInMediaStore(newPath,
							f.filePath, isDir);
				}
				return OperationEventListener.ERROR_CODE_SUCCESS;
				// if (needScan) {
	...
	public boolean isEnoughSpace(ArrayList<FileInfo> files, String dstFolder) {
		...
		if (spaceInfo != null) {
			...
			Log.i(LOG_TAG, "isEnoughSpace, freeSpace:" + freeSpace
					+ ", needSpace" + needSpace);
	...
	public void zipUpdate(String path) {
