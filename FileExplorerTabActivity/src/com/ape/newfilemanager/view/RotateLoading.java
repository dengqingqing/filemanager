...
public class RotateLoading extends View {
	...
	private void obtainStyleAttrs(AttributeSet attrs) {
		...
		color = typedArray.getColor(R.styleable.RotateLoading_loading_color,
				color);
		width = (int) typedArray.getDimension(
				R.styleable.RotateLoading_loading_width, width);
		shadowOffset = (int) typedArray.getDimension(
				R.styleable.RotateLoading_shadow_offset, shadowOffset);
		isStart = typedArray.getBoolean(
				R.styleable.RotateLoading_loading_visible, isStart);
		typedArray.recycle();
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		loadingRectF = new RectF(2 * width, 2 * width, w - 2 * width, h - 2
				* width);
		shadowRectF = new RectF(2 * width + shadowOffset, 2 * width
				+ shadowOffset, w - 2 * width + shadowOffset, h - 2 * width
				+ shadowOffset);
	}
